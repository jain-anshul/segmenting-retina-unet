...
model.summary()
plot(model, to_file='./'+name_experiment+'/nn'+'/'+name_experiment + '_model.png')   #check how the model looks like
json_string = model.to_json()
open('./'+name_experiment+'/nn'+'/'+name_experiment +'_architecture.json', 'w').write(json_string)

checkpointer = ModelCheckpoint(filepath='./'+name_experiment+'/nn'+'/'+name_experiment +'_best_weights.h5', verbose=1, monitor='val_loss', mode='auto', save_best_only=True) #save at each epoch if the validation decreased
...
model.save_weights('./'+name_experiment+'/nn/'+'/'+name_experiment +'_last_weights.h5', overwrite=True)
